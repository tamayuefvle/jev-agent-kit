...
HTTP 401 and 403 map to `AUTH_REJECTED`, 422 to `PROVIDER_INPUT_REJECTED`, and other non-200 statuses to `PROVIDER_HTTP_ERROR`. Only 429, 529, 502, 503, and 504 are retryable when `maxRetries` allows them. Network failures, contract failures, input rejection, cancellation, and timeout are not retried. Each response body is capped at 1 MiB while reading.
